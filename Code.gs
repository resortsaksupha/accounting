// ============================================================
//  RESORT STOCK SYSTEM — Google Apps Script Backend v2
//  แก้ไข: cors() function ที่ทำให้เกิด TypeError
// ============================================================
...
// ── JSON Response helper (แก้ไขแล้ว ไม่ใช้ setHeader) ──
...
function doGet(e) {
  ...
  try {
    switch (action) {
      ...
      default: result = { ok: true, message: 'Resort Stock API ready' };
    }
  } catch(err) {
...
function addStock(item) {
  ...
  const idx = data.findIndex(r => r.wh===item.wh && r.name===item.name && r.unit===item.unit);
...
function addWaste(item) {
  getSheet(SH.WASTE).appendRow([
    item.wh,item.name,item.qty,item.unit,item.reason||'',item.date||'',item.ts||Date.now()
  ]);
  const sh = getSheet(SH.STOCK);
  const data = sheetToArray(sh);
  const idx = data.findIndex(r => r.wh===item.wh && r.name===item.name && r.unit===item.unit);
  if (idx >= 0) {
    sh.getRange(idx+2,3).setValue(Math.max(0,(parseFloat(data[idx].qty)||0)-(parseFloat(item.qty)||0)));
  }
  return { ok:true };
}
...
function getSummary() {
  ...
  const todayExp = logs.filter(l=>l.type==='รับของ'&&l.date===todayStr).reduce((s,l)=>s+(parseFloat(l.price)||0),0);
  const todayRev = revenues.filter(r=>r.date===todayStr).reduce((s,r)=>s+(parseFloat(r.amount)||0),0);
  return { ok:true, data:{
    todayRevenue:todayRev, todayExpense:todayExp, todayProfit:todayRev-todayExp,
    totalStockItems:stock.length, lowStock:stock.filter(s=>parseFloat(s.qty)<=0).length
  }};
}
